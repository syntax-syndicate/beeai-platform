...
- **Hosts**: Applications or environments (IDEs, development tools, AI systems) that utilize ACP to access data and services through agents. A good example of a host is BeeAI UI that can be invoked by `beeai ui`.
...
ACP utilizes [JSON-RPC 2.0](https://www.jsonrpc.org/specification), that standardizes way to invoke functions on a remote server using JSON messages. JSON-RPC is a protocol specifically desinged for remote procderu calls (RPC) and dictates message format and request-response handling (making it explicitly a Layer 7 protocol).
...
<Note>
  We are implemneting a support for natural language interaction with agents in
  [ACP Alpha](/acp/alpha).
</Note>
...
- (Preferred) HTTP with Server-Sent Events (SSE): Optimal for remote transfer.
- Stdio: Optimal for local processes.
- (Under development) WebSockets: Optimal for bi-directional remote communication.

## ACP in OSI Model

Following is an example where ACP fits withing the Open Systems Interconnection (OSI) model. This is a simplified representation, showcasing the recommended use of ACP through HTTP with SSE.

Both ACP and JSON-RPC fit above the Applicaiton layer, mainly because it manages higher-level interactions.
