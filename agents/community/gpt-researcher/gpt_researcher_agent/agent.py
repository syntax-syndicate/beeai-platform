...
@server.agent(
    metadata=Metadata(
        ...
        env=[
            ...
            {"name": "LLM_MODEL_STRATEGIC", "description": "Strategic model to use from the specified OpenAI-compatible API."},
            {"name": "EMBEDDING_MODEL", "description": "Embedding model to use (see GPT Researcher docs for details)"},
        ]
    )
)
async def gpt_researcher(inputs: list[Message], context: Context) -> AsyncGenerator:
    print('inputs: ', inputs)
    ...
    LLM_MODEL = "llama3.1"
    os.environ["FAST_LLM"] = f"openai:{os.getenv('LLM_MODEL_FAST', LLM_MODEL)}"
    os.environ["SMART_LLM"] = f"openai:{os.getenv('LLM_MODEL_SMART', LLM_MODEL)}"
    os.environ["STRATEGIC_LLM"] = f"openai:{os.getenv('LLM_MODEL_STRATEGIC', LLM_MODEL)}"
    ...
    class CustomLogsHandler:
        async def send_json(self, data: dict[str, Any]) -> None:
            ...
            # match data.get("type"):
            #     case "logs":
            #         yield {"message": data.get("output", "")}
            #     case "report":
            #         output += data.get("output", "")
            #         yield {"message": data.get("output", "")}

    yield MessagePart(content="run")
    print('\n KK:: ', inputs[-1])
    researcher = GPTResearcher(query="Impact of climate change on global agriculture", report_type="research_report", websocket=CustomLogsHandler())
